...
(defun act-on-string-or-region (f &optional string from to)
  (interactive
   (if (use-region-p)
...
(defun from-spanish-to-english(string &optional from to)
  ...
  (let (workOnStringP inputStr outputStr)
    ...
    (setq inputStr (if workOnStringP string (buffer-substring-no-properties from to)))
    (setq outputStr
          (google-translate-json-translation (google-translate-request "Spanish" "English" inputStr)))
    (if workOnStringP
        outputStr
      (save-excursion
        (delete-region from to)
